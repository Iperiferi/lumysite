...
export default function PublicSiteClient({ data, subdomain }: { data: BusinessData; subdomain: string }) {
  ...
  const handleMenuDownload = async () => {
    ...
    } catch { downloadUrl = menu.pdf_url; }
    try {
      ...
      link.href = blobUrl; link.download = menuFilename;
      document.body.appendChild(link); link.click(); link.remove();
      window.setTimeout(() => window.URL.revokeObjectURL(blobUrl), 1000);
    } catch {
      const link = document.createElement('a');
      link.href = downloadUrl; link.rel = 'noopener noreferrer';
      document.body.appendChild(link); link.click(); link.remove();
    } finally {
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b shadow-sm">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t bg-white px-4 py-3 space-y-2">
            {navItems.map(n => (
              <a key={n.id} href={`#${n.id}`} onClick={() => setMobileMenuOpen(false)} className="block py-2 text-sm hover:opacity-70 transition">{n.label}</a>
            ))}
          </div>
        )}
      </nav>
                ...
                <div className="mb-12">
                  <h3 className="text-2xl font-bold mb-6" style={{ color: accent }}>
                    {isSectionEnabled('accommodations') && accommodations.length > 0 && isSectionEnabled('experiences') && experiences.length > 0
                      ...
                      : isSectionEnabled('accommodations') && accommodations.length > 0 ? t('nav.accommodations', lang) : t('nav.experiences', lang)}
                  </h3>
                  <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="max-w-5xl mx-auto px-4">
              ...
              {menu.pdf_url && (
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <button type="button" onClick={handleMenuDownload} disabled={isDownloadingMenu}
          ...
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-16">
              ...
              <div className="space-y-3 md:pl-12">
                {business.address && <div className="flex items-start gap-2"><MapPin className="w-5 h-5 mt-0.5 shrink-0" /><span>{business.address}</span></div>}
                {business.phone && <div className="flex items-center gap-2"><Phone className="w-5 h-5 shrink-0" /><a href={`tel:${business.phone}`} className="hover:underline">{business.phone}</a></div>}
                {business.email && <div className="flex items-center gap-2"><Mail className="w-5 h-5 shrink-0" /><a href={`mailto:${business.email}`} className="hover:underline">{business.email}</a></div>}
              </div>
            </div>
            {faq.length > 0 && (
              <div className="mt-12">
        ...
        <section id="kontakt" className="py-20">
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="space-y-3 text-lg">
              {business.phone && <div className="flex items-center gap-3"><Phone className="w-5 h-5" /><a href={`tel:${business.phone}`} className="hover:underline">{business.phone}</a></div>}
              {business.email && <div className="flex items-center gap-3"><Mail className="w-5 h-5" /><a href={`mailto:${business.email}`} className="hover:underline">{business.email}</a></div>}
              {business.address && <div className="flex items-center gap-3"><MapPin className="w-5 h-5" /><span>{business.address}</span></div>}
            </div>
            {socialLinks.length > 0 && (
              <div className="flex gap-4 mt-6">
                {socialLinks.map(s => (
                  <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition" style={{ color: accent }} aria-label={s.label}>{s.icon}</a>
                ))}
              </div>
            )}
            {mapSrc && (
      ...
      <footer className="border-t py-8 text-center text-sm text-muted-foreground">
        {socialLinks.length > 0 && (
          <div className="flex justify-center gap-4 mb-4">
            {socialLinks.map(s => (
              <a key={s.label} href={s.url} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition text-foreground" aria-label={s.label}>{s.icon}</a>
            ))}
          </div>
        )}
